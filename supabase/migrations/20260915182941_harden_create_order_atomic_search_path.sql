-- Migration 022: Harden create_order_atomic with explicit search_path and drop obsolete overloaded signature
-- Resolves Supabase Security Advisor warning (function_search_path_mutable)

-- 1. Drop obsolete 14-parameter overload to eliminate RPC signature ambiguity
DROP FUNCTION IF EXISTS public.create_order_atomic(
  text, text, text, text, text, text, text, text, text, numeric, numeric, text, jsonb, text
);

-- 2. Recreate the 18-parameter atomic order creation function with strict search_path = public, pg_temp
