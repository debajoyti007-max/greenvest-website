import{p as e}from"./index-DzIk8HH7.js";function t(t){let n=t.items.map(e=>`<tr><td>${e.emoji} ${e.name} (G${e.grade})</td><td>${e.qty}</td><td>৳${e.unitPrice}</td><td>৳${e.unitPrice*e.qty}</td></tr>`).join(``),r=t.deliverySlot?e[t.deliverySlot].en:`—`,i=`<!DOCTYPE html><html><head><title>Invoice ${t.id}</title>
  <style>
